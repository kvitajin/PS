...
/////////////////////////kopirovano z : https://gist.github.com/mfontanini/92a8b4a286c3dc42d432ce0956c25d71/////////////
...
//        const char tmp=words[i].c_str();
    int siz = words[0].length() + 1;
    std::string tmp;
    char wordArray[siz];
    strcpy(wordArray, words[0].c_str());
    auto hash = ConstexprHashes::md5(wordArray);
    std::stringstream s;
    s << std::hex << std::setw(2) << std::setprecision(2);
    for (auto i : hash) {
        s<< (static_cast<int>(i) & 0xff);
    }
    s>>tmp;
    std::cout <<tmp<< std::endl;
//    }
